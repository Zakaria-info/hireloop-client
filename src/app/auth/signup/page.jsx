...
// HeroUI v3 - Core button component only
import { Button } from '@heroui/react';
// Gravity UI Icons 
import { ArrowLeft, Envelope, Key, Person, CircleCheck, TriangleExclamation } from '@gravity-ui/icons';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.error) {
        setIsLoading(false);
        setError(response.error.message || 'Registration failed. Check details.');
      } else {
        setIsLoading(false);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-4 relative font-sans text-white">
      ...
      <button
        ...
        type="button"
      >
        <ArrowLeft width={16} height={16} className="transform group-hover:-translate-x-1 transition-transform" />
        Return
      </button>
      ...
      <article className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl overflow-hidden p-8 space-y-6">
        ...
        {error && (
          <section className="flex items-center gap-3 bg-red-950/40 border border-red-900/50 p-4 rounded-xl text-sm text-red-400 animate-fadeIn" role="alert">
        ...
        {success && (
          <section className="flex items-center gap-3 bg-emerald-950/40 border border-emerald-900/50 p-4 rounded-xl text-sm text-emerald-400 animate-fadeIn" role="alert">
